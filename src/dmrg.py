...
class HEffective(scipy.sparse.linalg.LinearOperator):
    ...
    Inherits from `scipy.sparse.linalg.LinearOperator` to allow for efficient
    diagonalization with `scipy.sparse.linalg.eigsh`.
    As with Lanczos, we don't build the diagonal matrix. All we need is to know
    how to apply the operator to a vector, which is done in `_matvec`.

    As Heff inherits from `LinearOperator`, it has the attributes
    `shape`, `dtype`, and `matvec` (which is implemented as `_matvec`).
    """
